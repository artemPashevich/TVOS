...
//    static let _API_VERSION = "/panel_pro/api"
    ...
    var backendServerUrl: String
    var _tokens: Tokens?
    var onNeedHelp: (() -> Void)?
    var onTokenChanged: ((Tokens) -> Void)?
    
    init(backendServerUrl: String, onTokenChanged: ((Tokens) -> Void)? = nil, onNeedHelp: (() -> Void)? = nil) {
        self.backendServerUrl = backendServerUrl
...
//        }
    ...
    func addDevice(login: String, password: String, name: String, completion: @escaping (DeviceInfo?) -> Void) {
        ...
        let url = URL(string: Fetcher.getBackendEndpoint + "/client/devices/add")
